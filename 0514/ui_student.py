...
class StudentWindow(wx.Dialog):
    def __init__(self, parent, title, userid):
        ...
        self.updateBtn = wx.Button(panel, wx.ID_ANY, '退选')
        ...
        def populate_course_list(self):
            ...
            for grade in grade_list:
                ...
                self.listGrade.SetItem(index, 6, grade[6])
                index += 1
        ...
        def onGradeList(self, e):
            ...
            self.inputTextScore.SetValue(self.listGrade.GetItem(index, 5).GetText())
        ...
        def onDelete(self, e):
            ...
            jxbid = self.inputTextJxbID.GetValue()
            # 删除记录
            data.delete_jxb(jxbid, self.userid)
            # 初始化界面
            self.refresh_screen()
